...
tests check exactly that, against the console repo's own copy of the file, so
the two cannot drift apart silently.

| Field | Where it comes from |
| --- | --- |
| `metrics.cpu_pct`, `load1/5/15`, `mem_*`, `swap_used_mb`, `net_*_bps` | `psutil`, i.e. `/proc`. Inside an add-on container those files describe the **host**, so these need no API at all. |
...
| `containers` | one row per add-on from `GET /addons`, with `cpu_pct` / `mem_mb` from `GET /addons/<slug>/stats` for **started** add-ons only |
...
| `ingest_token` | *(empty)* | **Required.** The console's per-host token for `ha`. While it is empty the add-on runs but pushes nothing, and says so once per interval. |
